...
- **`apps.py`**: Defines `get_build_app()` (python\_app for system building), `get_grompp_app()` and `get_mdrun_app()` (bash\_apps for GROMACS). All conditional logic (GPU, MPI, thread counts, restart flags) is resolved in Python before generating the bash script.
