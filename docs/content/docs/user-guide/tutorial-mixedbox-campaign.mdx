...
3. optionally run the GROMACS simulation chain
...
## 4. Optional: run the GROMACS simulation chain

If you want to continue through a full simulation campaign, run the simulation pipeline:

```bash
mdfactory simulate tutorial_campaign/mixedbox_chain_length_series_20.yaml
```

For SLURM clusters, generate a config and pass it:

```bash
mdfactory config slurm
mdfactory simulate \
  tutorial_campaign/mixedbox_chain_length_series_20.yaml \
  --slurm slurm_executor.yaml
```

<Callout type="info" title="SLURM configuration">
The `mdfactory config slurm` wizard auto-discovers your cluster's partitions, accounts, and hardware. See [Running on HPC Clusters](/docs/user-guide/running-on-hpc) for details.
</Callout>

This executes the fixed simulation sequence:
